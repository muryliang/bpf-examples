...
int load_prog(int ifidx) {
		...
		xdp_opts.open_filename = "xskfwd_kern.o";
		xdp_opts.prog_name = "xdp_fwd_af_prog";
		xdp_opts.opts = &opts;
		...
		err = xdp_program__attach(prog, ifidx, XDP_MODE_UNSPEC, 0);
		if (err) {
			libxdp_strerror(err, errmsg, sizeof(errmsg));
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < n_ports; i++) {
		...
		if (!ifindex) {
			printf("can not get ifidx of %s\n", ports[i]->params.iface);
			return -1;
		}
		int mapfd = load_prog(ifindex);
		int ret = xsk_socket__update_xskmap(ports[i]->xsk, mapfd);
		if (ret)
			return -1;
	}
	printf("All ports attaching program successfully.\n");
